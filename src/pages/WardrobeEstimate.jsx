...
export default function WardrobeEstimate() {
  ...
  function handleNext() {
    if (step === 1 && !estimateData.height) return;
    if (step === 2 && !estimateData.wardrobeType) return;
    if (step === 3 && !estimateData.material) return;
    setStep((prev) => prev + 1);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result.success) {
        setStatus('success');
        setEstimateData({ height: '', wardrobeType: '', material: '' });
  ...
  return (
    <main className="wardrobe-estimate">
      <div className="wardrobe-estimate__header">
        <h1>
          Get your free wardrobe estimate in <span>under 30 seconds!</span>
        </h1>
        <p>Tell us about your wardrobe preferences.</p>
        <div className="wardrobe-estimate__step">
          STEP <strong>{step}</strong> OF 4
        </div>
      </div>
      ...
      <div className="wardrobe-estimate__container">
        ...
        {step === 1 && (
          <section className="wardrobe-step">
            <div className="wardrobe-step__content">
              <span className="wardrobe-step__number">01</span>
              <h2>What is the height of your wardrobe?</h2>
              <p>Select the approximate wardrobe height.</p>
              ...
              <div className="height-options">
                {[4, 5, 6, 7, 8, 9].map((height) => (
                  <button
                    ...
                    onClick={() => handleEstimateChange('height', `${height} ft`)}
                  >
                    {height} ft
                  </button>
                ))}
        ...
        {step === 2 && (
          <section className="wardrobe-step">
            <div className="wardrobe-step__content">
              <span className="wardrobe-step__number">02</span>
              <h2>What type of wardrobe do you want?</h2>
              <p>Choose the wardrobe opening style.</p>
              ...
              <div className="wardrobe-type-options">
                {wardrobeTypes.map((type) => (
                  <button
                    ...
                    onClick={() => handleEstimateChange('wardrobeType', type)}
                  >
                    <div className="wardrobe-icon">{type === 'Sliding' ? '⇆' : '↔'}</div>
                    <strong>{type}</strong>
                    <span>{type === 'Sliding' ? 'Space-saving sliding doors' : 'Classic opening doors'}</span>
                  </button>
                ))}
              </div>

              <div className="wardrobe-navigation">
                <button type="button" className="wardrobe-back-button" onClick={handleBack}>
                  ← BACK
                </button>
                <button
                  type="button"
        ...
        {step === 3 && (
          <section className="wardrobe-step">
            <div className="wardrobe-step__content">
              <span className="wardrobe-step__number">03</span>
              <h2>What type of material do you want?</h2>
              <p>Select the material or finish you prefer.</p>
              ...
              <div className="material-options">
                {materials.map((material) => (
                  <button
                    ...
                    onClick={() => handleEstimateChange('material', material)}
                  >
                    {material}
                  </button>
                ))}
          ...
          <section className="wardrobe-contact-step">
            <div className="wardrobe-contact-card">
              <div className="wardrobe-contact-info">
                ...
                <span className="wardrobe-contact-tag">FREE QUOTE</span>
                <h2>Your dream wardrobe is almost ready</h2>
                <p>Share your contact details and our team will help you with the next steps.</p>
                ...
                <div className="wardrobe-summary">
                  ...
                  <div>
                    <span>Wardrobe Type</span>
                    <strong>{estimateData.wardrobeType}</strong>
                  </div>
                  <div>
                    <span>Material</span>
                    <strong>{estimateData.material}</strong>
                  </div>
                </div>
              </div>

              <form className="wardrobe-contact-form" onSubmit={handleSubmit} noValidate>
                <h3>Get your free quote</h3>
                ...
                <div className="wardrobe-contact-buttons">
                  ...
                  <button
                    type="submit"
                    className="wardrobe-submit-button"
                    disabled={status === 'sending' || !isContactStepValid()}
                  >
                ...
                {status === 'success' && (
                  <div className="wardrobe-success">
                    Our team will contact you soon. Redirecting you to the home page...
                  </div>
                )}
                {status === 'error' && (
                  <div className="wardrobe-error">Something went wrong. Please try again.</div>
                )}
              </form>
            </div>
          </section>
